...
def set_session_cookies(response: Response, token_value: str):
    # Correctif 4.5.1 : plus de double-submit CSRF.
    # L'origine est contrôlée par le middleware et le cookie de session est SameSite.
    response.set_cookie(
    ...
    # Nettoie les anciens cookies CSRF éventuellement conservés par le navigateur.
    response.delete_cookie(CSRF_COOKIE, path="/")

def clear_session_cookies(response: Response):
    response.delete_cookie(SESSION_COOKIE, path="/")
    response.delete_cookie(CSRF_COOKIE, path="/")
    response.delete_cookie("trekmap_session", path="/")
    response.delete_cookie("trekmap_csrf", path="/")
...
async def security_middleware(request: Request, call_next):
    ...
    try:
        ...
        raise
    response.headers["X-TrekMap-Version"] = APP_VERSION
